...
If you need to run a 2-dimensional table, you can execute a SPC command
using `redatam.query`. The next exmaple, run a crosstab
`person.ecivil by person.sexo`, next rename the columns- of the
...
Finally, if you need to make a crosstable, you can use R built-in
functions:
...
We can use the `pivottabler` library for building a crosstab. In the
next exmaple, we generate a 3-dimensional Redatam crosstab
`freq person.sexo by person.ecivil by person.alfab` and then generate a
pivot-table:
...
*If you want to render the pivot table, please uncomment the last line
of code*
...
If you wnat to export to an EXCEL file, you can use the `openxlsx`
package:
...
In the next block of code, you can find the whole program that generates
